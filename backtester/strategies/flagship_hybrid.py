"""Flagship strategy: a faithful port of the Round-2 competition submission.

Osmium is made with a hybrid fair value -- a Kalman filter fused with a
Bayesian precision-weighted mid, nudged by order-flow imbalance (OFI) and an
inventory-risk term -- then quoted with an elastic peg around 10,000 and volume
laddering.  Pepper is a pure buy-and-hold to the position limit.

Only the import path changed from the original submission; the trading logic is
preserved verbatim so the backtester measures the real strategy.
"""
...
PEPPER_LIMIT = 80
OSMIUM_LIMIT = 80
FV = 10000  # absolute peg for Osmium
...
class FlagshipHybrid:
    name = "flagship_hybrid"
    ...
    def run(self, state: TradingState):
        if state.traderData:
            try:
                ...
                for k, v in saved.get("history", {}).items():
                    self.history[k] = v
            except Exception:
                pass
        ...
        for product, order_depth in state.order_depths.items():
            ...
            # Intarian pepper root: pure buy & hold
            ...
            # Osmium: Bayesian + Kalman + OFI hybrid
            if product == "ASH_COATED_OSMIUM":
                ...
                if "OFI_STATE" not in self.history:
                    self.history["OFI_STATE"] = {
                        "prev_bid": best_bid, "prev_bid_vol": bid_vol,
                        "prev_ask": best_ask, "prev_ask_vol": ask_vol,
                    }
                ofi_state = self.history["OFI_STATE"]
                prev_bid = ofi_state["prev_bid"]
                prev_bid_vol = ofi_state["prev_bid_vol"]
                prev_ask = ofi_state["prev_ask"]
                prev_ask_vol = ofi_state["prev_ask_vol"]

                if best_bid > prev_bid:
                    e_b = bid_vol
                elif best_bid == prev_bid:
                    e_b = bid_vol - prev_bid_vol
                else:
                    e_b = -prev_bid_vol
                ...
                kalman_fair = x_new
                kalman_variance = max(1e-6, P_new)
                ...
                mr_signal = fair_price - mid
                combined_signal = abs(mr_signal + (0.15 * ofi))
                if combined_signal > 2.0:
                    EDGE = 0.5
        ...
        return result, 0, json.dumps({"history": self.history})

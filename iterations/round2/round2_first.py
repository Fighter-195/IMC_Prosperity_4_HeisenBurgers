...
# BAYESIAN VARIANCE WINDOW (Optimized to 20 for +0.657 correlation)
VAR_WINDOW = 20
...
class Trader:
    ...
    def bid(self) -> int:
        """
        Calculated Optimal EV Bid: 4100 XIRECs.
        Based on a data-derived True Value (V) of ~4930. 
        EV Maximization (B* ~3835) + Mass A Shrinkage Safety Margin (+246) = ~4082.
        Rounded to 4100 for robust median clearance.
        """
        return 4100
    ...
    def run(self, state: TradingState):
        ...
        for product, order_depth in state.order_depths.items():
            ...
            if product == "ASH_COATED_OSMIUM":
                ...
                # ------------------------------------------------------------
                # 2. ORDER FLOW IMBALANCE (OFI) CALCULATION
                # ------------------------------------------------------------
                ...
                e_b = 0
                if best_bid > prev_bid: e_b = bid_vol
                elif best_bid == prev_bid: e_b = bid_vol - prev_bid_vol
                else: e_b = -prev_bid_vol

                e_a = 0
                if best_ask < prev_ask: e_a = ask_vol
                elif best_ask == prev_ask: e_a = ask_vol - prev_ask_vol
                else: e_a = -prev_ask_vol
                ...
                # ------------------------------------------------------------
                # 4. BAYESIAN CONFIDENCE WEIGHTING
                # ------------------------------------------------------------
                if len(hist) > 1:
                ...
                # ------------------------------------------------------------
                # 8. MAKER LOGIC: Bypass + Elastic Peg + Laddering
                # ------------------------------------------------------------

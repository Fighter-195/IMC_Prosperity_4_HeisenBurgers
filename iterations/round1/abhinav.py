...
class Trader:
    ...
    def execution_engine(
        ...
        for product, fv in fair_values.items():
            ...
            # ------------------------------------------------------------------
            # STRATEGY B: TOMATOES (The Kelp "WallMid" Strategy)
            # ------------------------------------------------------------------
            elif product == 'TOMATOES':
                buy_capacity = limit - current_pos      
                sell_capacity = -limit - current_pos 

                # 1. Calculate the Dynamic Fair Price (Wall Mid)
                # If the book is completely empty on one side, fallback to the other
                ...
                if best_bid_book and best_ask_book:
                    ...
                    continue # No data to trade on this tick

                # Phase 1: Market Taking (Immediate Favorable Trades)
                # Take asks strictly below the wall mid, take bids strictly above.
                if order_depth.sell_orders and buy_capacity > 0:
                    for ask_price in sorted(order_depth.sell_orders.keys()):
                ...
                # Overbid/Undercut existing liquidity.
                # Crucially, we use math.floor/ceil on the wall_mid to ensure we 
                # never quote aggressively across the fair price line.
                passive_bid = min(best_bid_book + 1, math.floor(wall_mid))
                passive_ask = max(best_ask_book - 1, math.ceil(wall_mid))

                if current_pos >= skew_threshold:
                    # Inventory too high: neutralize at zero edge relative to current price
                    zero_edge_ask = math.ceil(wall_mid)
                    if sell_capacity < 0:
                        product_orders.append(Order(product, zero_edge_ask, sell_capacity))
                    if buy_capacity > 0:
                        product_orders.append(Order(product, passive_bid, buy_capacity))

                elif current_pos <= -skew_threshold:
                    # Inventory too low: neutralize at zero edge relative to current price
                    zero_edge_bid = math.floor(wall_mid)
                    if buy_capacity > 0:
                        product_orders.append(Order(product, zero_edge_bid, buy_capacity))
                    if sell_capacity < 0:
                        product_orders.append(Order(product, passive_ask, sell_capacity))

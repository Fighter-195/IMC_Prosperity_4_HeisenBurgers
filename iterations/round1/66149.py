    ...
    def execution_engine(self, state: TradingState, market_regime: str) -> Dict[str, List[Order]]:
        ...
        for product in state.order_depths.keys():
            ...
            # ── TOMATOES: Avellaneda-Stoikov + Sizing + Validated Base ───────
                ...
                # Base Quotes derived from Reservation Price & Dynamic Spread
                passive_bid = min(best_bid_book + 1, math.floor(reservation_price - dynamic_half_spread))
                passive_ask = max(best_ask_book - 1, math.ceil(reservation_price + dynamic_half_spread))

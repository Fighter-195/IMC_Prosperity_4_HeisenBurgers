...
HYPERPARAMETERS = {
    ...
    "TOMATOES_INITIAL_PHI2": 0.25,
    
    # Online Machine Learning Params
    "AR2_LEARNING_RATE": 0.04,     # How fast the bot updates its brain
    "INV_RISK_AVERSION": 0.03,     # How heavily inventory shifts the fair price
    
    "TOMATOES_SKEW_DIVISOR": 30.0,
...
# --- 4. ASYMMETRIC TAKER MARGINS ---
                base_margin = 1.0 
                inv_factor = 0.015 
                
                # If current_pos is positive (+50): buy_margin gets larger, sell_margin gets smaller
                # If current_pos is negative (-50): buy_margin gets smaller, sell_margin gets larger
                raw_buy_margin = base_margin + (current_pos * inv_factor)
                raw_sell_margin = base_margin - (current_pos * inv_factor)

                # Cap margins between 0.1 (desperate to trade) and 3.0 (refusing to trade)
                buy_margin = max(0.1, min(3.0, raw_buy_margin))
                sell_margin = max(0.1, min(3.0, raw_sell_margin))

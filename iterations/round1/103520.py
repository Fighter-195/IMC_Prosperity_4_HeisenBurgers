...
# =========================================================================
# ROUND 1 SUBMISSION PARAMETERS (Dynamic Learner Edition)
# =========================================================================
HYPERPARAMETERS = {
    "PEPPER_ROOT_PANIC_THRESHOLD": 65,
    
    # These are now INITIAL seeds. The bot will evolve these live.
    "OSMIUM_INITIAL_PHI1": -0.35,
    "OSMIUM_INITIAL_PHI2": 0.35,
    ...
    "OSMIUM_SKEW_DIVISOR": 34.0,
    "OSMIUM_SKEW_POWER": 2.2,
...
class Trader:
    def __init__(self):
        ...
        # Dynamic Brain State
        self.phi1 = HYPERPARAMETERS["OSMIUM_INITIAL_PHI1"]
        self.phi2 = HYPERPARAMETERS["OSMIUM_INITIAL_PHI2"]
    ...
    def execution_engine(self, state: TradingState) -> Dict[str, List[Order]]:
        ...
        for product in state.order_depths.keys():
            ...
            # ── INTARIAN_PEPPER_ROOT: Spread-Aware Pennying ───────────────────────
            if product == 'INTARIAN_PEPPER_ROOT':
                ...
                # SAFE Maker Logic (Protected against NoneTypes)
                ...
                if safe_best_ask - safe_best_bid > 1:
                    passive_bid = min(safe_best_bid + 1, math.floor(true_price) - 1)
                    passive_ask = max(safe_best_ask - 1, math.ceil(true_price) + 1)
                else:
                    passive_bid = min(safe_best_bid, math.floor(true_price) - 1)
                    passive_ask = max(safe_best_ask, math.ceil(true_price) + 1)

                em_panic = HYPERPARAMETERS["PEPPER_ROOT_PANIC_THRESHOLD"]

                if current_pos >= em_panic:
                    if sell_capacity < 0: product_orders.append(Order(product, math.ceil(true_price), sell_capacity))
                    if buy_capacity > 0:  product_orders.append(Order(product, passive_bid, buy_capacity))
                elif current_pos <= -em_panic:
                    if buy_capacity > 0:  product_orders.append(Order(product, math.floor(true_price), buy_capacity))
                    if sell_capacity < 0: product_orders.append(Order(product, passive_ask, sell_capacity))
                else:
                    if buy_capacity > 0:  product_orders.append(Order(product, passive_bid, buy_capacity))
                    if sell_capacity < 0: product_orders.append(Order(product, passive_ask, sell_capacity))

            # ── ASH_COATED_OSMIUM: Dynamic LMS AR(2) + Inventory Pricing ──────────────
                ...
                if best_bid_book is not None and best_ask_book is not None:
                    ...
                    continue # Skip tick entirely if order book is empty
                ...
                if len(self.history["ASH_COATED_OSMIUM"]) >= 4:
                    p_t = self.history["ASH_COATED_OSMIUM"][-1]
                    p_t1 = self.history["ASH_COATED_OSMIUM"][-2]
                    p_t2 = self.history["ASH_COATED_OSMIUM"][-3]
                    p_t3 = self.history["ASH_COATED_OSMIUM"][-4]
                    ...
                    # Normalized LMS to prevent volatility explosions
                ...
                skew_power = HYPERPARAMETERS["OSMIUM_SKEW_POWER"]
                skew_divisor = HYPERPARAMETERS["OSMIUM_SKEW_DIVISOR"]
